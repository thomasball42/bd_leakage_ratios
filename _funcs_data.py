...
def yield_kg_km2(
    ydat: pd.DataFrame, items: list[str], country_code: int, cropdb: pd.DataFrame
    ...
    if len(yields) == 0:
        item_codes = cropdb[cropdb.Item.isin(items)].Item_Code
        mask: Any = (ydat["Area Code"] == country_code) & (
...
def path_walking(rpath: str, comm_of_interest: str, coi_code: int) -> pd.DataFrame:
    ...
    for path in os.listdir(rpath):
        if os.path.isdir(os.path.join(rpath, path)):
            try:
                xdf: pd.DataFrame = pd.read_csv(
                    os.path.join(rpath, path, "xdf.csv"), index_col=0
                )
                cof: pd.DataFrame = xdf[
                    xdf["Item"].astype(str).str.contains(comm_of_interest, case=False)
                ]
                ccof: pd.DataFrame = cof[cof["Producer_Country_Code"] == coi_code]
                current_country_iso: str = path.upper()
                if len(ccof) > 0:
                    import_ratio = (
                        cof.groupby("Country_ISO")["provenance"].sum()
                        / cof["provenance"].sum()
                    )
                    cof["export_val"] = cof.loc[
...
def leakage_calculation(outdf: pd.DataFrame, crop_bd: pd.DataFrame) -> pd.DataFrame:
    ...
    :param outdf: the dataframe that the bd_leakage column is being appended to
    :param crop_bd: the dataframe that is used to calculate the bd_val
    :return outdf: the dataframe that the bd_leakage column is being appended to"""
